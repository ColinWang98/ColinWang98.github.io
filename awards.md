---
layout: default
title: "Awards & Skills"
permalink: /awards/
author_profile: true
classes: subpage
---

# Awards & Skills

## Technical Skills

<div class="skills-grid">
  <div class="skill-category">
    <h4><i class="fas fa-code"></i> Programming & Development</h4>
    <ul>
      <li><strong>Unity</strong> - Game development and VR/AR applications</li>
      <li><strong>Blender</strong> - 3D modeling and animation</li>
      <li><strong>Maya</strong> - Advanced 3D modeling and rendering</li>
      <li><strong>Rhino</strong> - Parametric design and CAD modeling</li>
      <li><strong>Figma</strong> - UI/UX design and prototyping</li>
    </ul>
  </div>

  <div class="skill-category">
    <h4><i class="fas fa-paint-brush"></i> Design & Creative</h4>
    <ul>
      <li><strong>Photoshop</strong> - Digital image editing and manipulation</li>
      <li><strong>Aquarelle</strong> - Traditional watercolor painting</li>
      <li><strong>Mark Painting</strong> - Mixed media art techniques</li>
      <li><strong>Sketch</strong> - Concept art and illustration</li>
      <li><strong>Photography</strong> - Professional photography and editing</li>
    </ul>
  </div>

  <div class="skill-category">
    <h4><i class="fas fa-chart-bar"></i> Data & Analysis</h4>
    <ul>
      <li><strong>Stata</strong> - Statistical analysis and econometrics</li>
      <li><strong>GIS Software</strong> - Geographic information systems</li>
      <li><strong>Research Methods</strong> - Quantitative and qualitative analysis</li>
      <li><strong>Data Visualization</strong> - Charts, graphs, and infographics</li>
    </ul>
  </div>

  <div class="skill-category">
    <h4><i class="fas fa-globe"></i> Languages</h4>
    <ul>
      <li><strong>English</strong> - IELTS overall 7.0 (Professional proficiency)</li>
      <li><strong>Chinese</strong> - Native speaker (Mandarin)</li>
      <li><strong>Technical Writing</strong> - Academic and technical documentation</li>
    </ul>
  </div>
</div>

## Academic Achievements

### Language Certification
<div class="paper-box">
  <div class="paper-box-text">
    <h3>IELTS Academic</h3>
    <p><strong>Overall Score:</strong> 7.0</p>
    <p>Demonstrates strong English proficiency for academic and professional purposes. This certification validates the ability to communicate effectively in international academic and research environments.</p>
    <ul>
      <li>Reading: Advanced comprehension of complex texts</li>
      <li>Writing: Professional academic writing skills</li>
      <li>Listening: Effective understanding of lectures and discussions</li>
      <li>Speaking: Confident oral communication abilities</li>
    </ul>
  </div>
</div>

## Creative Portfolio

### Photography
<div class="project-item">
  <h4>Signed Contributor - tuchong.com</h4>
  <p>Recognized photographer with a portfolio of landscape, architectural, and cultural heritage photography. Specializing in documenting traditional architecture and cultural sites.</p>
  <div class="project-tags">
    <span class="badge">Photography</span>
    <span class="badge">Cultural Heritage</span>
    <span class="badge">Architecture</span>
  </div>
</div>

### Literary Works
<div class="project-item">
  <h4>Published Writer - Jianshu.com</h4>
  <p>Author of over 10,000 words of poetry and creative writing, exploring themes of cultural heritage, urban development, and human connection with the built environment.</p>
  <div class="project-tags">
    <span class="badge">Creative Writing</span>
    <span class="badge">Poetry</span>
    <span class="badge">Cultural Studies</span>
  </div>
</div>

## Professional Development

### Certifications & Training
- **Virtual Reality Development**: Advanced training in VR application development
- **Urban Planning Software**: Certified in professional planning and GIS tools
- **Cultural Heritage Documentation**: Specialized training in heritage preservation methods
- **Research Methodology**: Advanced training in academic research techniques

### Workshops & Conferences
- **International Conference on Virtual Heritage** - Presenter (2024)
- **Urban Planning Innovation Workshop** - Participant (2023)
- **Digital Preservation Techniques** - Advanced Training (2023)
- **HCI Research Methods** - Graduate Level Course (2024)

## Recognition & Achievements

### Academic Excellence
- **High Academic Performance**: Consistent high GPA across both undergraduate and graduate studies
- **Research Contributions**: Multiple academic publications and conference presentations
- **Leadership Recognition**: Successfully led student organizations and research projects

### Creative Recognition
- **Photography Portfolio**: Featured work in online photography communities
- **Cultural Documentation**: Recognized for heritage preservation photography
- **Literary Contributions**: Published creative works exploring cultural themes 